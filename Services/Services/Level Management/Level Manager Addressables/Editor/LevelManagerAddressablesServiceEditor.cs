    ...
    public class LevelManagerAddressablesServiceEditor : Editor
    {
        ...
        private void OnEnable()
        {
            ...
            _list.drawElementCallback = (Rect rect, int index, bool isActive, bool isFocused) =>
            {
                ...
                var prefab = element.FindPropertyRelative("levelPrefab");
                ...
                EditorGUI.PropertyField(fieldRect, prefab);
                addToRandomList.boolValue = EditorGUI.Toggle(toggleRect, addToRandomList.boolValue);
                if (GUI.Button(buttonRect, "Spawn"))
                {
                    (target as LevelManagerAddressablesService).SpawnLevelInDebugMode(prefab.boxedValue as AssetReference);
                }
            };
        }
